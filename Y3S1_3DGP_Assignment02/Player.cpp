...
void CPlayer::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	if (nCameraMode == THIRD_PERSON_CAMERA || nCameraMode == SPACESHIP_CAMERA)	// SPACESHIP_CAMERA	THIRD_PERSON_CAMERA
	{
...
CLevel1Player::CLevel1Player(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	////비행기 메쉬를 생성한다. 
	//CMesh* pAirplaneMesh = new CAirplaneMeshDiffused(pd3dDevice, pd3dCommandList,
	//	20.0f, 20.0f, 4.0f, XMFLOAT4(0.0f, 0.5f, 0.0f, 0.0f));
	...
	// 정육면체 메쉬 생성 (12x12x12)
	CCubeMeshDiffused* pCubeMesh = new CCubeMeshDiffused(pd3dDevice, pd3dCommandList, 5, 5, 10);
...
CCamera* CLevel1Player::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	DWORD nCurrentCameraMode = (m_pCamera) ? m_pCamera->GetMode() : 0x00;
	if (nCurrentCameraMode == nNewCameraMode) return(m_pCamera);
	switch (nNewCameraMode)
...
void CLevel1Player::OnPrepareRender()
{
	CPlayer::OnPrepareRender();
	//비행기 모델을 그리기 전에 x-축으로 90도 회전한다. 
	/*XMMATRIX mtxRotate = XMMatrixRotationRollPitchYaw(XMConvertToRadians(90.0f), 0.0f, 0.0f);
...
CLevel2Player::CLevel2Player(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	// 정육면체 메쉬 생성 (12x12x12)
	...
	//플레이어의 카메라를 스페이스-쉽 카메라로 변경(생성)한다.
	m_pCamera = ChangeCamera(SPACESHIP_CAMERA, 0.0f);		// THIRD_PERSON_CAMERA
...
CCamera* CLevel2Player::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	DWORD nCurrentCameraMode = (m_pCamera) ? m_pCamera->GetMode() : 0x00;
	if (nCurrentCameraMode == nNewCameraMode) return(m_pCamera);
	switch (nNewCameraMode)
...
void CLevel2Player::OnPrepareRender()
{
	CPlayer::OnPrepareRender();
}
